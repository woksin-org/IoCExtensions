...
public partial class the_scenario
{
	...
	protected static void AssertConfiguration<T>(T config, T expected)
	{
		AssertCorrectType(config);
		Assert.Equivalent(expected, config);
	}

	protected static void AssertCorrectType<T>(T service)
		=> AssertCorrectType(service, typeof(T));

	protected static void AssertCorrectType<T>(T service, Type implementationType)
	{
